...
import { fadeUp, staggerContainer, staggerPassThrough, viewportOnce } from '../motion/variants'
...
const Contact = ({ data }: ContactProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!web3Key) {
      ...
      window.location.href = `mailto:${data.email}?subject=${encodeURIComponent(`Portfolio: ${formData.name}`)}&body=${body}`
      setStatus('success')
    ...
    try {
      ...
      try {
        ...
        setStatus('error')
        setErrorDetail('Invalid response from mail service. Try the email link below.')
        return
      }
      if (res.ok && json.success === true) {
        ...
        setStatus('error')
        setErrorDetail(json.message || 'Something went wrong. Try the email link below.')
      }
    } catch {
      setStatus('error')
    ...
    <section id="contact" className="contact">
      <div className="container">
        ...
        >
          ...
          <motion.div className="contact-content" variants={staggerPassThrough}>
            ...
            <motion.form variants={fadeUp} className="contact-form" onSubmit={handleSubmit}>
              <input
                type="text"
                name="company"
